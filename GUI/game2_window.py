...
import args
from Q_Loader2 import CountryDescriptionReader
...
class GameWindow(QMainWindow, Ui_GameWindow):
    ...
    def keyPressEvent(self, event):
        # Check if the pressed key is 'A' (Qt.Key_A)
        ...
        if event.key() == Qt.Key_D:
            print("D PRESSED")
            # self.next_btn.clicked.connect(lambda: self.next_round(0))
            self.next_btn.click()

    # def call_next_round_call(self):
    #     self.next_round(0)
...
"border-radius: 4;")
    ...
    def start_timer(self,time_add=0):
        # time_add--> used to add aditional time 
        self.time_left = args.ROUND_TIME + time_add
